...
\[
  ...
  \varepsilon y(x-yw).
\]
Here ``to first order'' means that terms of size \(\varepsilon^2\) and smaller are ignored. When the learning rate is small, those terms are much smaller than the leading term. So Oja's rule is a local, online approximation to Hebbian learning plus Euclidean normalization.
