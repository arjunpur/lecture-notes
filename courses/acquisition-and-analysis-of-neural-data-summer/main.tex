...
In data analysis one usually computes a binned approximation. Divide $[0,T]$ into $M=T/\Delta t$ bins and let
\[
x_i=
\begin{cases}
...
\end{cases}
\]
For small enough $\Delta t$, the discretized response is approximately $\rho_i=x_i/\Delta t$. The raw autocorrelation pair count at lag $m$ is
\[
N_m:=\sum_{i=1}^{M} x_i x_{i+m}.
\]
...
\]
It counts each spike with itself and therefore produces a large central peak. This peak is mathematically meaningful, but for visualization it is often omitted so that nonzero-lag structure is visible.

\subsubsection{Expected autocorrelogram under the homogeneous Poisson model}

For a homogeneous Poisson process, the nonzero-lag raw pair count has expectation approximately
\[
\mathbb{E}[N_m]\approx n\,\frac{n-1}{T}\Delta t,
\qquad 0<|m|\ll M.
\]
The factor $(n-1)\Delta t/T$ is the conditional probability that, given one spike, another spike falls in the bin at lag $m$. Thus the normalized histogram
\[
H_m:=
\frac{1}{T}
\left(
...
\]
This equation compresses two facts. For $\tau\ne 0$, independent Poisson spikes have no excess correlation after subtracting the mean-rate product. At $\tau=0$, the same spike is perfectly coincident with itself, producing the delta peak.
