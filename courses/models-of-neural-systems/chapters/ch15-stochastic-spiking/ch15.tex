...
\textit{Variability is part of the spike-generation model, not merely measurement noise. This chapter introduces the point-process statistics needed to quantify stochastic firing, count variability, spectra, coherence, and signal transmission.}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/mns-stochastic-spiking.png}
\caption{Point-process views of neural variability. Raster plots, PSTHs, ISI distributions, and Fano factors summarize the same spike train at different levels, while conditional intensity makes spike-history effects explicit.}
\label{fig:stochastic-spiking}
\end{figure}
