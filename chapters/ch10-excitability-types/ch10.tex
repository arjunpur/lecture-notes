...
\textit{Not all neurons start firing the same way. Some can fire at arbitrarily low rates, while others jump abruptly to a minimum nonzero frequency. This distinction traces back to the type of bifurcation that creates the limit cycle: a saddle-node on an invariant circle produces Type~I (integrator) behavior, while a Hopf bifurcation produces Type~II (resonator) behavior. These two classes differ in everything from threshold geometry to coding strategy.}
...
As the injected current increases, a neuron transitions from silence to repetitive firing. The transition occurs when the stable resting state loses stability and a limit cycle appears. But there are fundamentally different ways this can happen, and the mechanism determines the neuron's computational character.

The two main bifurcation types are the \emph{saddle-node on an invariant circle}\index{saddle-node on invariant circle} (SNIC) and the \emph{Hopf bifurcation}\index{Hopf bifurcation}. Each creates a limit cycle, but with profoundly different properties.
