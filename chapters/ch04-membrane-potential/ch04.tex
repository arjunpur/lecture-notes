...
\begin{aside}[The membrane is a capacitor]
...
\begin{aside}[Diffusion as a random walk]
...
\begin{aside}[The Einstein relation]
...
\]
The left side is $\frac{RT}{zF}\frac{\dd}{\dd x}\ln C$. Integrate across the membrane from inside ($x = 0$) to outside ($x = d$):
\[
...
\begin{aside}[What the Nernst equation means]
...
\begin{aside}[Example: computing reversal potentials]
...
\begin{aside}[Common mistake: Nernst applies to one ion at a time]
...
\begin{aside}[GHK as a weighted average]
...
By Kirchhoff's law, the total current into the cell must equal the sum of all currents leaving through channels plus the current charging the capacitor. We adopt the sign convention that positive current flows \emph{into} the cell, and ionic currents are positive when flowing \emph{outward}.
...
\begin{aside}[Reading the membrane equation]
...
\begin{aside}[Example: typical membrane parameters]
...
\begin{aside}[Connection: the leaky integrate-and-fire model]
...
\begin{aside}[Common mistake: shunting inhibition]
Adding more open channels \emph{decreases} the time constant---the membrane responds faster but the voltage excursions become smaller ($R_{\text{eff}} = 1/g_{\text{tot}}$ decreases). Inhibition with $\Erev{\text{inh}} \approx \Vrest$ does not hyperpolarize, but it ``shunts'' excitatory drive by dividing the input resistance.
\end{aside}
